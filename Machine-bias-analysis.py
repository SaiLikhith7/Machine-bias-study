...
if __name__ == "__main__":
    ...
    from (select uid, attr, val from Pro_Publica ) as A  ,(select uid, attr, val from Pro_Publica) as C \
     where A.uid=C.uid and A.attr<C.attr and A.attr!='vdecile' and C.attr!='vdecile'"                     
    F5 = spark.sql(query5)
    ...
    query9 = "select F8.attr1,F8.val1,F8.attr2,F8.val2,F8.attr3, F8.val3, F8.supp, (F8.supp/F6.supp) as conf from F6,F8 where F6.attr1=F8.attr1 and F6.val1=F8.val1 and F6.attr2=F8.attr2 and F6.val2=F8.val2  and (F8.supp/F6.supp)>= "+str(conf);                     
    F9 = spark.sql(query9)
